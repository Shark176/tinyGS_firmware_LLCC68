...
template<>
int16_t RadioHal<LLCC68>::begin(float freq, float bw, uint8_t sf, uint8_t cr, uint8_t syncWord, int8_t power, uint16_t preambleLength, uint8_t gain, float tcxoVoltage)
{
    if (power>=17) radio->setCurrentLimit(150);
    return radio->begin(freq, bw, sf, cr, syncWord, power, preambleLength);
}
template<>
int16_t RadioHal<LLCC68>::begin()
{
    return radio->begin();
}
...
template<>
int16_t RadioHal<LLCC68>::beginFSK(float freq, float br, float freqDev, float rxBw, int8_t power, uint16_t preambleLength, bool enableOOK, float tcxoVoltage, bool useRegulatorLDO)
{
    if (power>=17) radio->setCurrentLimit(150);
    return radio->beginFSK(freq, br, freqDev, rxBw, power, preambleLength, tcxoVoltage, useRegulatorLDO);
}
...
template<>
float RadioHal<LLCC68>::getRSSI(bool packet,bool skipReceive)
{
    return radio->getRSSI(packet);
}
...
template<>
float RadioHal<LLCC68>::getFrequencyError(bool autoCorrect)
{
    return radio->getFrequencyError();
}
...
template<>
void RadioHal<LLCC68>::setDio0Action(void (*func)(void))
{
    radio->setPacketReceivedAction(func);
}
...
template<>
int16_t RadioHal<LLCC68>::startReceive(uint8_t len, uint8_t mode)
{
    return radio->startReceive(len);
}
...
template<>
int16_t RadioHal<LLCC68>::autoLDRO()
{
    return radio->autoLDRO();
}
...
template<>
int16_t RadioHal<LLCC68>::forceLDRO(bool enable)
{
    return radio->forceLDRO(enable);
}
...
template<>
int16_t RadioHal<LLCC68>::fixedPacketLengthMode(uint8_t len)
{
    return radio->fixedPacketLengthMode(len);
}
...
template<>
int16_t RadioHal<LLCC68>::setCRC(uint8_t len,	uint16_t initial , uint16_t polynomial , bool inverted )
{
    return radio->setCRC(len,initial,polynomial,inverted);
}
...
template<>
int16_t RadioHal<LLCC68>::setEncoding(uint8_t encoding) 
{
    if (encoding == 10)
        encoding = 1;
