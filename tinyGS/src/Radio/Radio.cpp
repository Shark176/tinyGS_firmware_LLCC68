...
void Radio::init()
{
 ...
 switch (board.L_radio) {
    ...
    case RADIO_SX1268:
      ...
      break;
    case RADIO_SX1262:
      radioHal = new RadioHal<LLCC68>(new Module(board.L_NSS, board.L_DI01, board.L_RST, board.L_BUSSY, spi, SPISettings(2000000, MSBFIRST, SPI_MODE0)));
      moduleNameString="LLCC68";
      break;
    case RADIO_SX1280:
...
int16_t Radio::remoteSetFreqOffset(char *payload, size_t payload_len)
{
  ...
  status.radio_ready = false;
  CHECK_ERROR(radioHal->sleep());  //  mandatory if FastHop isn't ON.
